...
def solve_sys_pde_lhs(array_b, array_i, array_s, array_l, sys_params, sol_params, in_circle_array,
    ...
    new_array_b = impose_Neumann_boundary(new_array_b)
    new_array_i = impose_Neumann_boundary(new_array_i)
...
def run_and_save_sys(array_b, array_i, array_s, array_l, sys_params, sol_params):
    ...
    pSmad_delta_values = np.zeros((Lx, Ly), dtype=np.float64) # Not updating
    for i in range(1, Lx-1):
        for j in range(1, Ly-1):
            # check if the mesh cell is in the circle
    ...
    for t in range(T):
        ...
        # The cell colony starts growing after a given T_growth and starts
        # growing at a uniform rate
        if growth == 0:
            # check time to grow or not
            if t >= sol_params[7]:
                growth = 1

        else:
            # Already growing -> Update the in_circle array
            # Update the radius
            sol_params[4] += sol_params[8]*dt
            R = int(sol_params[4])
            for i in range(1, Lx-1):
                for j in range(1, Ly-1):

                    # check if the mesh cell is in the circle
                    in_circle_array[i, j] = (1 if (((i - Lx//2)**2 + (j - Ly//2)**2) <= R**2) 
                                             else 0)
